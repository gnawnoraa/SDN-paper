...
class Shortest_Route(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.stats = self.network_monitor.stats
    ...
    def get_least_used_path(self, graph, src_dpid):
        if graph is None:
            self.logger.debug("Graph is empty.")
            return None
        length = len(graph)
        ...
        if src_dpid not in nodes:
            ...
            nodes.remove(src_dpid)

        # while nodes:
        #     nodes.remove

        #get datapath loading
    ...
    def _packet_in_handler(self, ev):
        ...
        #self.logger.info("sw%s: PACKET_IN %s->%s at port %s - %s", datapath.id, eth_src, eth_dst, in_port, pkt)

        if isinstance(arp_pkt, arp.arp):
            self.logger.info("[ARP]sw%s: PACKET_IN %s->%s at port %s", datapath.id, eth_src, eth_dst, in_port)
        ...
        elif eth_dst == mac.BROADCAST_STR:
            self.logger.info("[BROADCAST]sw%s: PACKET_IN %s->%s at port %s", datapath.id, eth_src, eth_dst, in_port)
            for dpid in self.access_ports:
                for port in self.access_ports[dpid]:
                    if (dpid, port) not in self.access_table.keys():
                        actions = [parser.OFPActionOutput(port)]
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            self.logger.info("[IPv4]sw%s: PACKET_IN %s->%s at port %s", datapath.id, eth_src, eth_dst, in_port)
