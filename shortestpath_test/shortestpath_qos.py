...
class MultipathForwarding(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, priority, match, actions, idle_timeout=10, hard_timeout=180, table_id=1):
        """
    ...
    def install_mpls_path_flow(self, path, msg):
        ...
        if len(path) > 3:    # different switch
            for i in range(1, len(path) - 1):
                ...
                if switch == path[1]:
                    ...
                    actions.append(parser.OFPActionSetField(mpls_label=mpls_value))
                    ...
                    match_fields['eth_dst'] = eth.dst
                    match_fields['eth_type'] = ether.ETH_TYPE_MPLS  #match_fields['eth_type'] = eth.ethertype
                    match_fields['mpls_label'] = mpls_value
                    match = parser.OFPMatch(**match_fields)

                    if switch == path[-2]:
                        # egress switch -> pop MPLS
                        actions = []
                        actions.append(parser.OFPActionPopMpls(ether.ETH_TYPE_IP))      # defalut use IPv4 0x0800
                        actions.append(parser.OFPActionOutput(out_port))
                    else:
        ...
        out_actions.insert(0, parser.OFPActionSetQueue(1))
    ...
    def install_path_flow(self, path, msg):
        ...
        out_actions = [parser.OFPActionSetQueue(1, type_=ofproto.OFPAT_SET_QUEUE)]
        # out_actions = []
