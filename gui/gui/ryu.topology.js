...
var topo = {
    ...
    delete_nodes: function (nodes) {
        for (var i = 0; i < nodes.length; i++) {
            ...
            if (node_index != null) this.nodes.splice(node_index, 1);
        }
        this.refresh_node_index();
    },
    delete_links: function (links) {
        for (var i = 0; i < links.length; i++) {
            ...
            if (link_index != null) this.links.splice(link_index, 1);
        }
    },
...
setTimeout(get_hosts, 2000);
function get_hosts() {
    $.get("http://" + location.host + "/v1.0/topology/hosts", function(data) {
        $.each(data, function(no, host) {
            var hosts = [];
            hosts.push({"dpid":host.mac,"ports":"1"});
            topo.delete_nodes(hosts);
            topo.add_nodes(hosts);

            links = {0: {"src": host.port, "dst":{"dpid": host.mac, "port_no":"1"}}, length: 1}
            topo.delete_links(links);
            topo.add_links(links);

            elem.update();
            // {"src":{"hw_addr":"1e:ba:71:c3:84:27","name":"s2-eth3","port_no":"00000003","dpid":"0000000000000002"},"dst":{"hw_addr":"4e:be:28:9b:03:81","name":"s4-eth3","port_no":"00000003","dpid":"0000000000000004"}}
        })
    });
}