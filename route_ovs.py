...
from ryu.lib.packet import packet, ethernet, arp, ipv4, ipv6, lldp, tcp, udp, dhcp, icmp
...
import network_monitor_ovs as network_monitor
import classification
import qos_control_ovs as qos_control
...
class MultipathController(ControllerBase):
    ...
    def set_admin_rule(self, req, **kwargs):
        multipath_app = self.multipath_app
...
class MultipathForwarding(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        ### [180606] source and destination mac address of the ethernet packet
        dst = eth.dst
        src = eth.src
        ### [180606]

        # source and destination port initialization
        src_port = 0
        dst_port = 0
        ...
        #if dpid not in self.net or dpid == 2 or dpid == 4:
        #    return None
        if dpid not in self.net:
            self.logger.info("dpid not in self.net")
            return None
        ...
        if src not in self.net:
            # qos_control.update_queue(dpid, self.data_share, self.qos_share, in_port)
        ...
        if arp_pkt:
            ...
            print "[arp table]: " + json.dumps(self.arp_table)
        # </editor-fold>
    ...
    def install_new_flow_path(self, path, msg):
        '''
            path=[src_mac, dpid1, dpid2 ... dst_mac]
        '''
        if msg.datapath.id not in path:
            print "the dpid:%s not in" % msg.datapath.id, path
            return None
        path_id = None
        ...
        self.add_flow(datapath, 1, match, actions, idle_timeout=30, hard_timeout=0)
    ...
    def classifiy(self, msg):
        elephants_port = [5001, 5002, 5003, 5004]   
        qos_high_port = []
        qos_high_tcp_port = []
        ...
        if pkt_ipv4 is not None:
            if pkt_ipv4.proto == inet.IPPROTO_TCP:
                ...
                label = DATA_ELEPHANTS_LABEL if pkt_tcp.dst_port in elephants_port else DATA_MICE_LABEL
    ...
    def load_balance_path(self, src, dst):
        ...
        for path in all_paths:
            ...
            load_list = [self.network_monitor.get_link_loading_mbps(path[i], self.net[path[i]][path[i + 1]]['port']) for i in range(1, len(path) - 2)]
            if len(load_list) == 0:
                loading[tuple(path)] = 0
            else:
                loading[tuple(path)] = max(load_list)
            # total = 0
            # for i in range(1, len(path)-1):
            #     switch = path[i]
            #     next_switch = path[i+1]
        ...
        # if len(loading) == 0:
            # return []
        lowest_path = list(min(loading, key=loading.get))
        # self.logger.info("All paths loading: %s", loading)
        #self.logger.info("Path %s -> %s loading(bits):%s via", src, dst, min(loading))
        self.logger.info("[LoadBalancing]Path %s -> %s loading(bits):%s via %s", src, dst, loading[tuple(lowest_path)], lowest_path)
    ...
    def get_match_l4(self, msg): ### [180607]
        """
        ...
        MPEG_DASH_port = [2222, 8181]
        ftp_port = [20, 21, 989, 990]
        dhcp_port = [67, 68]
        http_port = [80]
        https_port = [443]
        rtp_port = [5004, 5005]
        ...
        if pkt_ipv4 is None:
            ...
            if pkt_ipv4.proto == inet.IPPROTO_ICMP:
                ...
                self.logger.info("[ICMP packet] From %s.", pkt_ipv4.src)

            elif pkt_ipv4.proto == inet.IPPROTO_TCP:
                self.logger.debug("Got a TCP packet")

                # store source & destination port number in match field
                pkt_tcp = pkt.get_protocol(tcp.tcp)
                match_fields['tcp_src'] = pkt_tcp.src_port
                match_fields['tcp_dst'] = pkt_tcp.dst_port
                src_port = match_fields['tcp_src']
                dst_port = match_fields['tcp_dst']

                # print protocol tag on controller's screen
                if src_port in ftp_port or dst_port in ftp_port:
                    self.logger.info("[FTP packet]")
                elif src_port in http_port or dst_port in http_port:
                    self.logger.info("[HTTP packet]")
                elif src_port in https_port or dst_port in https_port:
                    self.logger.info("[HTTPS packet]")
                elif src_port in MPEG_DASH_port or dst_port in MPEG_DASH_port:
                    self.logger.info("[MPEG-DASH packet]")
                ...
                # store source & destination port number in match field
                pkt_udp = pkt.get_protocol(udp.udp)
                match_fields['udp_src'] = pkt_udp.src_port
                match_fields['udp_dst'] = pkt_udp.dst_port
                src_port = match_fields['udp_src']
                dst_port = match_fields['udp_dst']
                
                # print protocol tag on controller's screen
                if pkt_udp.dst_port in dhcp_port:
                    match_fields.pop('ipv4_dst', None)
                if src_port in rtp_port or dst_port in rtp_port:
                    self.logger.info("[RTP packet]")
