...
class QoS(object):
    ...
    def _set_qos(self, cookie, rest, waiters, vlan_id):
        ...
        #     self.ofctl.mod_flow_entry(self.dp, flow, cmd)
        # except:
        #     raise ValueError('Invalid rule parameter. flow=%s, cmd=%s' % (flow, cmd))
        ...
        for key, value in rest.items():
            if key in Match._CONVERT:
                if value in Match._CONVERT[key]:
                    ...
                    raise ValueError('Invalid rule parameter. key=%s : value=%s' % (key, value))
            else:
